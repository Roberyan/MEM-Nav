...
def origin_sem_map_vis(GT_map):
    ...
    cv2.imwrite('./tmp/img/origin_map.png',sem_map_gt, [int(cv2.IMWRITE_JPEG_QUALITY), 95])
...
def map_conversion(sem_map, start_x, start_y, start_o):
    ...
    for i in range(sem_map.shape[0]):
        ...
        loc_conversion = (a * cos + b * sin).astype(np.int32) + LOCAL_MAP_SIZE//2, (b * cos - a * sin).astype(np.int32) + LOCAL_MAP_SIZE//2
...
def data_check(scene_name):
    ...
    for i, eps in enumerate(eps_data):
        ...
        min_x, min_y = map_obj_origin / 100.0 # from cm to m metric
        ...
        stx = stx.astype(np.int32)
        sty = sty.astype(np.int32)
        ...
        ret, labels = cv2.connectedComponents(mx_crilce.astype(np.int328))
        mx_vis[labels==labels[240,240]] = [100, 100, 100]
        cv2.imwrite('./tmp/img/test.png',np.flipud(mx_vis), [int(cv2.IMWRITE_JPEG_QUALITY), 95])
        cv2.imwrite('./tmp/img/dist_map.png',np.flipud(dist_map_vis), [int(cv2.IMWRITE_JPEG_QUALITY), 95])
        pass
    # save_data
