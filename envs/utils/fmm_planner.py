...
class FMMPlanner:
    def __init__(self, traversible, scale=1, step_size=5):
        ...
        if scale != 1.0:
            ...
            self.traversible = np.copy(traversible)
    ...
    def set_goal(self, goal, auto_improve=False):
        ...
        goal_x, goal_y = int(goal[0] / (self.scale * 1.0)), int(goal[1] / (self.scale * 1.0))
    ...
    def set_multi_goal(self, goal_map, validate_goal=False):
        if self.scale != 1.0:
            goal_map = cv2.resize(
                goal_map,
                (goal_map.shape[1] // self.scale, goal_map.shape[0] // self.scale),
                interpolation=cv2.INTER_NEAREST,
            )
            goal_map = np.rint(goal_map)
        traversible_ma = ma.masked_values(self.traversible * 1, 0)
        if validate_goal:
            # Note: update goal map to contain only traversible regions
            # Otherwise, the masked regions of traversible_ma may be overwritten.
            goal_map = np.copy(goal_map)
            goal_map[self.traversible == 0] = 0
        ...
        # if self.scale != 1.:
        #     # Visualize outputs
        #     vis_map = np.zeros((*goal_map.shape, 3), dtype=np.uint8)
        #     vis_map[self.traversible == 1, :] = np.array([128, 128, 128])
        #     vis_map[goal_map == 1, :] = np.array([255, 255, 255])
        #     H, W = goal_map.shape
        #     vis_map[H // 2 - 1 : H // 2 + 2, W // 2 - 1 : W // 2 + 2, :] = np.array([255, 0, 0])
        #     cv2.imwrite('image.png', vis_map[..., ::-1])
        #     import pdb
        #     pdb.set_trace()
        return

    def get_short_term_goal(self, state, dthresh=0.25):
        ...
        if subset[self.du, self.du] < dthresh * 100 / 5.0:  # 25cm
            stop = True
        else:
            stop = False
