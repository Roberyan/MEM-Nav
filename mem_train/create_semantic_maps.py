...
def extract_scene_point_clouds(
    ...
    per_floor_wall_pc = extract_wall_point_clouds(
        glb_path, houses_dim_path, sampling_density=sampling_density, grid_size=GRID_SIZE
    )
...
def visualize_sem_map(sem_map, with_palette=True):
...
def convert_point_cloud_to_semantic_map(
    ...
    for obj_f in tqdm.tqdm(obj_files):
        ...
        for floor_id, floor_dims in per_floor_dims.items():
            ...
            map_semantic_rgb_with_palette = visualize_sem_map(map_semantic)
            map_semantic_rgb = visualize_sem_map(map_semantic, False)
