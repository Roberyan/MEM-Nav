...
from blip2_conditioned_VQVAE import create_MemMapVQVAE
...
if __name__ == "__main__":
    ...
    #####################################################################################################
    # load training model
    mem_generator, vqvae = create_MemMapVQVAE(model_config)

    # Initialize wandb.
    project_name = f"{train_config['model_name']}_{train_config['dataset_name']}"
    task_name = f"weighted_loss_no_wall(recon-vq-aux=1:0.7:0)(0 3 0 10)"
    wandb.init(
        project=project_name, 
        config=train_config, 
        name=task_name)
    
    # optimizer = optim.Adam([{"params": vqvae.encoder.parameters(), "lr": 1e-4},{"params": vqvae.decoder.parameters(), "lr": 1e-4},{"params": mem_generator.parameters(), "lr": 1e-5}], weight_decay=1e-5)
    optimizer = optim.AdamW(
        list(vqvae.parameters()) + list(mem_generator.parameters()),
    ...
    for epoch in range(start_epoch, train_config["num_epochs"]):
        ...
        vqvae.train()
        mem_generator.eval()  # Keep memory generator in eval mode
        epoch_loss = 0.0
